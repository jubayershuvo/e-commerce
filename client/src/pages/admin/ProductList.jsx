          ...
          >
            <div className="flex items-start space-x-4">
              ...
              <div className="text-sm text-gray-900 dark:text-gray-300">
                <h3 className="text-sm md:text-lg font-semibold">{product.title} (ID:{product._id})</h3>
                {product.discount > 0 ? (
                  <p className="text-xs text-green-500">
                    {product.discount}% Off
                  </p>
                ) : null}
                 {product.availableSize.length > 1 ? <p className="text-xs">Available Sizes: {product.availableSize.join(", ")}</p>: <p className="h-3"> </p>}
                {product.price !== product.regularPrice && (
                  <p className="md:text-sm text-xs font-semibold opacity-50 line-through">
                    {product.regularPrice?.toFixed(2)} BDT
                  </p>
                )}
                <p className="md:text-sm text-xs font-semibold">{product.price?.toFixed(2)} BDT</p>
              </div>
            </div>

            <div className="absolute right-4 bottom-4">
            <div className="flex items-center space-x-2 cursor-pointer">
              <PencilSquareIcon width={20} className="text-black dark:text-white"/>
            </div>
            </div>
          </div>
        ))
