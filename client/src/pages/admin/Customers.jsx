...
const CustomerListPage = () => {
  ...
  const { customers, admin } = useSelector((state) => state.adminAuth);
  ...
  const handleBan = async (id) => {
    ...
    try {
      ...
      const filteredCustomers = res.data.data.filter((cust=>(cust._id !== admin._id)))
        dispatch(setCustomers(filteredCustomers));
  ...
  const handleRole = async () => {
    if (password) {
      if (editType === "customer") {
        ...
        try {
          const res = await axios.post(`/admin/user/removeAdmin`, {
          ...
          dispatch(setCustomers(filteredCustomers));
          toast.success("Now the user is an admin", { id: loading });
          setEditCustomer({});
          setPassword("");
        } catch (error) {
          toast.error(error.response.data.message, { id: loading });
        ...
        const loading = toast.loading("Processing...");
        try {
          const res = await axios.post(`/admin/user/makeAdmin`, {
            _id: editCustomer._id,
            password: password,
          });
          const filteredCustomers = res.data.data.filter(
          ...
          dispatch(setCustomers(filteredCustomers));
          toast.success("Now the admin is an user.", { id: loading });
          setEditCustomer({});
          setPassword("");
        } catch (error) {
          toast.error(error.response?.data.message, { id: loading });
